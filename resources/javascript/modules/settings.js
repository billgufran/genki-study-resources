...
class SettingsSyncPanel {
    constructor() {
        ...
        this.handleSyncMetaChange = (event) => {
        ...
        window.addEventListener('genki-auth-state', this.handleAuthStateChange);
        window.addEventListener('genki-sync-meta', this.handleSyncMetaChange);
    }
    waitForSettings(attempt = 0) {
    ...
    wrapManager(settings) {
        const original = settings.manager;
...
function loadSyncMetaFromStorage() {
    try {
...
function formatLastSynced(meta) {
...
function getLatestTimestamp(meta) {
    if (!meta) {
        return null;
    }
    const candidates = [
